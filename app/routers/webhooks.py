...
async def receive_github(
    ...
    print(f"Verified GitHub event: {x_github_event} for repo id: {matched_webhook.repo_id if matched_webhook else 'unknown'}")
    ...
    if x_github_event in ["push","workflow_run","workflow_job"] and matched_webhook is not None:
        try:
            ...
            effective_token = settings.GITHUB_RUNNER_PAT or gh_token
            
            # Test the token has sufficient permissions
            try:
                print(f"WEBHOOKS: Testing token permissions before launching runner")
                import httpx
                headers = {"Authorization": f"Bearer {effective_token}", "Accept": "application/vnd.github+json"}
                async with httpx.AsyncClient() as client:
                    # Verify repo access
                    repo_resp = await client.get(f"https://api.github.com/repos/{repo.owner_login}/{repo.name}", headers=headers)
                    print(f"WEBHOOKS: Repo access check status: {repo_resp.status_code}")
                    if repo_resp.status_code != 200:
                        print(f"WEBHOOKS: WARNING - Runner token cannot access repo {repo.owner_login}/{repo.name}: {repo_resp.status_code}")
                        print(f"WEBHOOKS: Response body: {repo_resp.text}")
                    
                    # Get token scopes - First try the user endpoint
                    print(f"WEBHOOKS: Checking token scopes via user endpoint")
                    user_resp = await client.get("https://api.github.com/user", headers=headers)
                    
                    # Print all headers for debugging
                    print(f"WEBHOOKS: Response headers: {dict(user_resp.headers)}")
                    
                    # Extract scopes from headers
                    scope_header = user_resp.headers.get("x-oauth-scopes")
                    print(f"WEBHOOKS: Raw scope header: {scope_header}")
                    
                    if scope_header:
                        scopes = [s.strip() for s in scope_header.split(",") if s.strip()]
                        print(f"WEBHOOKS: Token scopes: {scopes}")
                    else:
                        # Try another endpoint to get scopes
                        print(f"WEBHOOKS: No scopes found in user response, trying rate_limit endpoint")
                        rate_resp = await client.get("https://api.github.com/rate_limit", headers=headers)
                        scope_header = rate_resp.headers.get("x-oauth-scopes")
                        print(f"WEBHOOKS: Rate limit scope header: {scope_header}")
                        scopes = [s.strip() for s in (scope_header or "").split(",") if s.strip()]
                        print(f"WEBHOOKS: Token scopes from rate_limit: {scopes}")
                    
                    # Check if we have necessary scopes
                    if not scopes or ("repo" not in scopes and "admin:org" not in scopes):
                        print(f"WEBHOOKS: WARNING - Token lacks required scopes for runner registration. Has: {scopes}")
                        print(f"WEBHOOKS: Runner registration will likely fail!")
                    else:
                        print(f"WEBHOOKS: Token has sufficient scopes for runner registration")
            except Exception as e:
                print(f"WEBHOOKS: Error checking token permissions: {e}")
                import traceback
                print(f"WEBHOOKS: Traceback: {traceback.format_exc()}")
                
            env_vars = [
                {"name": "GH_OWNER", "value": repo.owner_login},
                {"name": "GH_REPO", "value": repo.name},
                {"name": "GITHUB_RUNNER_PAT", "value": effective_token},
